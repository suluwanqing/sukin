...
在 `stack` 模式下，卡片以扇形展开。
您可以通过 `stack-extraction` 属性控制卡片的“抽出”方式：

- `'click'`: 点击任意卡片会使其从扇形中“原地抽出”，再次点击或点击其他卡片会收回。
- `'hover'`: 鼠标悬停在卡片上时，该卡片会抽出。
- `'none'`: 卡片不会被抽出，`active-index` 对应的卡片仅高亮。
...
| `card-width`                | 自定义卡片宽度（像素值，或包含单位的 CSS 字符串）     | `number \| string`                             | `undefined` |
| `card-height`               | 自定义卡片高度（像素值，或包含单位的 CSS 字符串）     | `number \| string`                             | `undefined` |
| `stack-offset`              | [stack] 卡片扇形展开时的水平偏移量                    | `number`                                       | `80`        |
| `stack-rotate`              | [stack] 卡片扇形展开时的旋转角度                      | `number`                                       | `6`         |
| `stack-extracted-offset`    | [stack] 被选中卡片向上抽出的垂直偏移量                | `number`                                       | `40`        |
| `stack-extraction`          | [stack] 卡片抽出/高亮的方式                           | `enum` - `'click' \| 'hover' \| 'none'`        | `'click'`   |
...
| `show-indicators`           | [peek] 是否显示底部导航圆点                           | `boolean`                                      | `false`     |
